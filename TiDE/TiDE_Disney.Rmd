...
getSymbols("DIS", from = "2000-01-01", to = "2025-01-01", auto.assign = TRUE)

# DIS está en un xts, lo convertimos a data.frame
df <- data.frame(Date = index(DIS), Close = as.numeric(Cl(DIS)))
...
ggplot(df_real_vs_pred, aes(x = Date)) +
  ...
  labs(title = "Precio de cierre de Dinsey: predicción vs real",
       y = "Precio",
       x = "Fecha",
       color = "") +
  theme_minimal() +

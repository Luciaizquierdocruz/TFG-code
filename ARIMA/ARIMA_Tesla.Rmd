...
vector <- as.vector(TSLA$TSLA.Close)
train <- close["/2023"]
test <- close["2024"]

ts_data <- ts(vector, frequency = 252, start = c(2011, 1))
train_ts <- ts(as.numeric(train), frequency = 252, start = c(2011, 1))
...
No presenta ninguna componente, ni regular ni estacional. Se debe a que es totalmente inesperado, ruido blanco. 

**m1: ruido blanco**

```{r}
m1 <- arima(d1_lnserie, order = c(0,0,0)) 
round(m1$coef/sqrt(diag(m1$var.coef)),2)
```

Intercept signif --> miramos si drift ayuda
 
```{r}
# Con constante
fit_with_const <- Arima(ln_data, order = c(0,1,0), include.constant = TRUE)
round(fit_with_const$coef/sqrt(diag(fit_with_const$var.coef)),2)
AIC(fit_with_const)
# Sin constante
fit_no_const <- Arima(ln_data, order = c(0,1,0), include.constant = FALSE)
round(fit_no_const$coef/sqrt(diag(fit_no_const$var.coef)),2)
AIC(fit_no_const)
```
...
```{r}
mod <- Arima(log(train_ts), order = c(0,1,0), include.constant = TRUE)  
...
ts.plot(ts_data, lower_ts, upper_ts, forecast_ts,
        ...
        col = c(1,4,4,2),
        xlim = c(2024, 2025),
        main = "Predicción TSLA 2024 con ARIMA(0,1,2)",
        ylab = "Precio cierre")
...
ts.plot(ts_data, lower_ts, upper_ts, forecast_ts,
        ...
        col = c(1,4,4,2),
        xlim = c(2024, 2025),
        main = "Predicción Tesla 2024 con ARIMA(0,1,0)",
        ylab = "Precio cierre")
abline(v=2021:2025, lty=3, col=4)
...
```{r}
ln_data<-log(train_ts)
modelo <- Arima(ln_data, order = c(0,1,0), include.constant = TRUE)  #log(train_ts)
validation(modelo)
```
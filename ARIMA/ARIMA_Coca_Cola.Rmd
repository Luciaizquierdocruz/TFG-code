---
title: "ARIMA y autoARIMA Coca-Cola"
...
plot(d1_lnserie,main="d1d252serie")
abline(h=0)
mean(d1_lnserie)
```
...
```{r}
(m1 <- arima(d1_lnserie, order = c(1,0,0)))
round(m1$coef/sqrt(diag(m1$var.coef)),2)
```


```{r}
(m1.1 <- arima(d1_lnserie, order = c(0,0,0)))
...
```{r}
(m2 <- arima(d1_lnserie, order = c(3,0,0)))
round(m2$coef/sqrt(diag(m2$var.coef)),2)
```


```{r}
(m2.1 <- arima(d1_lnserie, order = c(2,0,0)))
round(m2.1$coef/sqrt(diag(m2.1$var.coef)),2)
```
...
```{r}
(m3 <- arima(d1_lnserie, order = c(0,0,2)))
...
```{r}
(m4 <- arima(d1_lnserie, order = c(0,0,4)))
...
```{r}
(m5 <- arima(d1_lnserie, order = c(1,0,2)))
round(m5$coef/sqrt(diag(m5$var.coef)),2)
```

```{r}
(m5.1 <- arima(d1_lnserie, order = c(1,0,1)))
round(m5.1$coef/sqrt(diag(m5.1$var.coef)),2)
```

**Modelo 6: AR(1)MA(4)**

```{r}
(m6 <- arima(d1_lnserie, order = c(1,0,4)))
round(m6$coef/sqrt(diag(m6$var.coef)),2)
```

**Modelo 7: AR(3)MA(2)**

```{r}
(m7 <- arima(d1_lnserie, order = c(3,0,2)))
round(m7$coef/sqrt(diag(m7$var.coef)),2)
```

```{r}
(m7.1 <- arima(d1_lnserie, order = c(2,0,2)))
round(m7.1$coef/sqrt(diag(m7.1$var.coef)),2)
```

**Modelo 8: AR(3)MA(4)**

```{r}
(m8 <- arima(d1_lnserie, order = c(3,0,4))) # 3 0 4
round(m8$coef/sqrt(diag(m8$var.coef)),2)
```

## Elección del mejor modelo 

```{r}
#modelos con signif y que convergen bien
modelos <- c(m1.1$aic,m2.1$aic, m3$aic, m4$aic, m5.1$aic, m6$aic, m7.1$aic)
which.min(modelos)
min(modelos)
```

Mejor modelo --> Modelo 7.1: ARIMA(2,1,2)

## Predicción 

```{r}
mod <- arima(log(train_ts), order = c(2,1,2))  
...
ts.plot(ts_data, lower_ts, upper_ts, forecast_ts,
        ...
        col = c(1,4,4,2),
        xlim = c(2024, 2025),
        main = "Predicción Coca-Cola 2024 con ARIMA(2,1,2)",
        ylab = "Precio cierre")
...
ts.plot(ts_data, lower_ts, upper_ts, forecast_ts,
        ...
        col = c(1,4,4,2),
        xlim = c(2024, 2025),
        main = "Predicción Coa-Cola 2024 con ARIMA(2,1,2)",
        ylab = "Precio cierre")
abline(v=2021:2025, lty=3, col=4)
...
```{r}
modelo <- arima(ln_data, order = c(2,1,2))  #log(train_ts)
validation(modelo)
```

---
title: "ARIMA y autoARIMA Disney"
...
close <- DIS[, "DIS.Close"]
vector <- as.vector(DIS$DIS.Close)
train <- close["/2023"]
test <- close["2024"]
...
```{r}
#(m1 <- arima(d1_lnserie, order=c(1,0,0), seasonal=list(order=c(1,0,0), period=252)))
# compararemos el AIC (bondad del ajuste)
```

No converge, miramos si la parte regular sola ya explica la serie

```{r}
modelo_prueba <- auto.arima(ln_data, seasonal = FALSE)

# Gráfico completo de residuos
checkresiduals(modelo_prueba)
```

Si, no parece que haya suficientes residuos sin explicar.

Modelos solo con parte regular.

**m2:AR(1)**

```{r}
(m2 <- arima(d1_lnserie, order = c(1,0,0)))
...
```{r}
(m3 <- arima(d1_lnserie, order = c(0,0,1)))
...
```{r}
(m4 <- arima(d1_lnserie, order = c(0,0,2)))
...
```{r}
(m5 <- arima(d1_lnserie, order = c(1,0,1)))
...
```{r}
(m6 <- arima(d1_lnserie, order = c(1,0,2)))
...
modelos <- c(m2$aic,m3$aic, m4$aic,m5$aic)
which.min(modelos)
min(modelos)
```

Mejor modelo --> Modelo 4: ARIMA(0,1,2)
...
ts.plot(ts_data, lower_ts, upper_ts, forecast_ts,
        ...
        col = c(1,4,4,2),
        xlim = c(2024, 2025),
        main = "Predicción Disney 2024 con ARIMA(0,1,2)",
        ylab = "Precio cierre")
...
ts.plot(ts_data, lower_ts, upper_ts, forecast_ts,
        ...
        col = c(1,4,4,2),
        xlim = c(2024, 2025),
        main = "Predicción Disney 2024 con ARIMA(1,1,0)",
        ylab = "Precio cierre")
abline(v=2021:2025, lty=3, col=4)
...
```{r}
modelo_manual <- arima(ln_data, order = c(0,1,2))  #log(train_ts)
validation(modelo_manual)
```


```{r}
modelo_auto <- arima(ln_data, order = c(1,1,0))  #log(train_ts)
